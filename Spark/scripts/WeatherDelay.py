...
def weather_delay(data_source, output_uri):
    """
    ...
    """
    with SparkSession.builder.appName("Calculate weather delay").getOrCreate() as spark:
        ...
        weather_delay_data = spark.sql("""SELECT Year, avg((WeatherDelay/ArrDelay)*100) FROM delay_data GROUP BY Year""")
        # Write the results to the specified output URI
        weather_delay_data.write.option("header", "true").mode("overwrite").csv(output_uri)
...
if __name__ == "__main__":
    ...
    weather_delay(args.data_source, args.output_uri)
